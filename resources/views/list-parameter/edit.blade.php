...
@section('content')
    ...
    <div class="max-w-3xl space-y-6">
        <div>
            ...
            <p class="text-sm text-slate-500">Perbarui data master parameter.</p>
        </div>

        <div class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
            <form action="{{ route('list-parameter.update', $item) }}" method="POST" class="space-y-5">
                @csrf
                @method('PUT')
                ...
                <div class="grid gap-5 md:grid-cols-2">
                    <div>
                        ...
                        <input type="text" name="nama_parameter"
                            value="{{ old('nama_parameter', $item->nama_parameter) }}"
                    ...
                    <div>
                        ...
                        <input type="text" name="parameter_utama"
                            value="{{ old('parameter_utama', $item->parameter_utama) }}"
                    ...
                    <div>
                        ...
                        <input type="text" name="default_satuan"
                            value="{{ old('default_satuan', $item->default_satuan) }}"
                            class="mt-1 w-full rounded-md border-slate-300 p-2 text-sm shadow-sm">
                        ...
                        <select name="default_parameter_group_id"
                            ...
                            @foreach ($groups as $group)
                                <option value="{{ $group->id }}"
                                    @selected((string) old('default_parameter_group_id', $item->default_parameter_group_id) === (string) $group->id)>
                                    {{ $group->nama_group }}
                                </option>
                            @endforeach
                        </select>
