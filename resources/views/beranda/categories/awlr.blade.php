        ...
        <div class="col-span-12 md:col-span-8 space-y-3 md:border-r md:border-slate-200 md:pr-2">
            ...
            <div class="relative overflow-hidden rounded-lg bg-white">
                <div class="absolute left-2 top-2 space-y-2 text-[10px] font-semibold text-slate-600">
                    <div class="w-24 rounded-md border px-2 py-1 {{ $isOnline ? 'border-orange-200 bg-orange-50 text-orange-700' : 'border-slate-300 bg-white text-slate-500' }}">
                        DATA AIR TANAH<br><span class="text-slate-900">{{ $dataAir ?? '-' }}</span>
                        <span class="text-slate-500">m</span>
                    </div>
                    <div class="w-24 rounded-md border px-2 py-1 {{ $isOnline ? 'border-rose-200 bg-rose-50 text-rose-700' : 'border-slate-300 bg-white text-slate-500' }}">
                ...
                <div class="absolute right-2 top-2 space-y-2 text-[10px] font-semibold text-slate-600">
                    <div class="w-24 rounded-md border px-2 py-1 {{ $isOnline ? 'border-sky-200 bg-sky-50 text-sky-700' : 'border-slate-300 bg-white text-slate-500' }}">
                        MUKA AIR TANAH<br><span class="text-slate-900">{{ $mukaAir ?? '-' }}</span>
                        <span class="text-slate-500">m</span>
                    </div>
                    <div class="w-24 rounded-md border px-2 py-1 {{ $isOnline ? 'border-amber-200 bg-amber-50 text-amber-700' : 'border-slate-300 bg-white text-slate-500' }}">
                ...
                <div class="flex items-center justify-center p-4">
                    <svg width="180" height="280" viewBox="0 0 180 280" xmlns="http://www.w3.org/2000/svg">
                        <defs>
                            <linearGradient id="waterGradient-{{ $lg->id_logger }}" x1="0%" y1="0%" x2="0%" y2="100%">
                                <stop offset="0%" style="stop-color:#93C5FD;stop-opacity:1" />
                                <stop offset="50%" style="stop-color:#3B82F6;stop-opacity:1" />
                                <stop offset="100%" style="stop-color:#1E40AF;stop-opacity:1" />
                            </linearGradient>
                            <pattern id="wallPattern-{{ $lg->id_logger }}" width="10" height="10" patternUnits="userSpaceOnUse">
                            ...
                            <linearGradient id="wallGradient-{{ $lg->id_logger }}" x1="0%" y1="0%" x2="100%" y2="0%">
                        ...
                        <rect x="35" y="45" width="15" height="230" fill="url(#wallGradient-{{ $lg->id_logger }})" />
                        <rect x="50" y="45" width="15" height="230" fill="url(#wallPattern-{{ $lg->id_logger }})" />
                        <rect x="115" y="45" width="15" height="230" fill="url(#wallPattern-{{ $lg->id_logger }})" />
                        <rect x="130" y="45" width="15" height="230" fill="url(#wallGradient-{{ $lg->id_logger }})" />
                        ...
                        <ellipse cx="90" cy="{{ $waterStartY }}" rx="25" ry="3" fill="#60A5FA" opacity="0.6" />
