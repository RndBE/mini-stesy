        ...
        <div class="col-span-12 md:col-span-8 space-y-3 md:border-r md:border-slate-200 md:pr-2">
            ...
            <div class="relative rounded-lg bg-white">
                ...
                @php
                    ...
                    // === Leader line target positions (scaled 1.6x) ===
                    $waterSurfYScaled = round($wStartY * 1.6);
                    $sensorYScaled    = round($sensorY  * 1.6);
                    $pompaYScaled     = round($pompaY   * 1.6);
                    $wellLeftXScaled  = round($innerX * 1.6);
                    $wellRightXScaled = round(($innerX + $innerW) * 1.6);
                    $diagramWScaled   = round(137 * 1.6);  // ≈219
                    $diagramHScaled   = round($totalH * 1.6);

                    // Container layout
                    $cW      = 116;  // card width px
                    $cGap    = 14;   // gap between card and well
                    $wellOff = $cW + $cGap;
                    ...
                    $contH = max($diagramHScaled + 24, $elevSensY + $cH + 24);
                @endphp
                ...
                <div class="mx-auto py-3"
                    ...
                    <svg style="position:absolute; top:0; left:0; overflow:visible; pointer-events:none; z-index:50;"
                        ...
                        <line x1="{{ $wlcX }}" y1="{{ $waterSurfYScaled }}"
                            x2="{{ $leftCardEndX }}" y2="{{ $datAirMidY }}"
                            stroke="{{ $isOnline ? '#78716c' : '#9CA3AF' }}" stroke-width="1.5" />
                        ...
                        <line x1="{{ $wlcX }}" y1="{{ $sensorYScaled }}"
                            x2="{{ $leftCardEndX }}" y2="{{ $elevSensMidY }}"
                            stroke="{{ $isOnline ? '#E11D48' : '#9CA3AF' }}" stroke-width="1.5" />
                        ...
                        <line x1="{{ $wrcX }}" y1="{{ $waterSurfYScaled }}"
                            x2="{{ $rightCardEndX }}" y2="{{ $mukaAirMidY }}"
                            stroke="{{ $isOnline ? '#0EA5E9' : '#9CA3AF' }}" stroke-width="1.5" />

                        {{-- ELEVASI POMPA → pompa (right, DINAMIS dari sumur ke card tetap) --}}
                        @if ($hasPompa)
                            <circle cx="{{ $wrcX }}" cy="{{ $pompaYScaled }}" r="4"
                                fill="{{ $isOnline ? '#F59E0B' : '#9CA3AF' }}" />
                            <line x1="{{ $wrcX }}" y1="{{ $pompaYScaled }}"
                                x2="{{ $rightCardEndX }}" y2="{{ $elevPompaMidY }}"
                                stroke="{{ $isOnline ? '#F59E0B' : '#9CA3AF' }}" stroke-width="1.5" />
                        @endif
                    </svg>

                    {{-- WELL DIAGRAM --}}
                    <div style="position:absolute; left:{{ $wellOff }}px; top:0;">
                        <div style="width:{{ $diagramWScaled }}px; height:{{ $diagramHScaled }}px; display:flex; align-items:flex-start; justify-content:center;">
                            <div class="relative"
                                style="width:137px; height:{{ $totalH }}px; overflow:visible; transform:scale(1.6); transform-origin:top center;">

                                <img src="{{ asset('sumur/top_sumur.svg') }}" width="97" height="14" class=""
                                    style="position:absolute; top:{{ $topTop }}px; left:{{ $topOffX }}px; z-index:50; display:block; image-rendering:auto;"
                                    alt="Top Sumur" />

                                <img src="{{ asset('sumur/badan_sumur.svg') }}" width="137" height="{{ $badanH }}" class=""
                                ...
                                <svg style="position:absolute; top:0; left:0; z-index:15;" width="137"
                                    height="{{ $totalH }}" viewBox="0 0 137 {{ $totalH }}" xmlns="http://www.w3.org/2000/svg">
                                    <defs>
                                        <linearGradient id="wG-{{ $uid }}" x1="0%" y1="0%" x2="0%" y2="100%">
                                            <stop offset="0%"   style="stop-color:{{ $wSurf }};stop-opacity:1" />
                                            <stop offset="20%"  style="stop-color:{{ $wTop }};stop-opacity:1" />
                                            <stop offset="60%"  style="stop-color:{{ $wMid }};stop-opacity:1" />
                                            <stop offset="100%" style="stop-color:{{ $wBot }};stop-opacity:1" />
                                        </linearGradient>
                                        <clipPath id="wClip-{{ $uid }}">
                                            <rect x="{{ $innerX }}" y="{{ $wellTop }}" width="{{ $innerW }}" height="{{ $wellRange }}" />
                                        </clipPath>
                                    </defs>
                                    <rect x="{{ $innerX }}" y="{{ $wStartY }}" width="{{ $innerW }}" height="{{ $wHeight }}"
                                        fill="url(#wG-{{ $uid }})" opacity="{{ $wOpac }}" clip-path="url(#wClip-{{ $uid }})" />
                                    <ellipse cx="{{ $cx }}" cy="{{ $wStartY }}" rx="12" ry="1.5" fill="{{ $wSurf }}"
                                        opacity="{{ $isOnline ? '0.85' : '0.20' }}" />
                                </svg>

                                {{-- CASING DALAM --}}
                                <img src="{{ asset('sumur/dalam_sumur.svg') }}" width="29" height="{{ $dalH }}" class=""
                                ...
                                <svg style="position:absolute; top:0; left:0; z-index:94;" width="137"
                                    ...
                                    <image href="{{ asset('sumur/cap_sensor.svg') }}" x="{{ $capSensX }}"
                                        y="{{ $capSensY }}" width="{{ $capSensW }}" height="{{ $capSensH }}"
                                ...
                                <svg style="position:absolute; top:0; left:0; z-index:60;" width="137"
                                    ...
                                    <image href="{{ asset('sumur/line_sensor.svg') }}" x="{{ $sensorLineX }}"
                                        ...
                                        height="{{ $sensorLineH + 0.5 }}" preserveAspectRatio="none" opacity="{{ $sOpac }}" />
                                    <image href="{{ asset('sumur/kepala_sensor.svg') }}" x="{{ $sensorHeadX }}"
                                        y="{{ $sensorHeadY }}" width="{{ $sensorHeadW }}" height="{{ $sensorHeadH }}"
                                        preserveAspectRatio="xMidYMid meet" opacity="{{ $sOpac }}" />
                                    @if ($hasPompa)
                                        <image href="{{ asset('sumur/line_pompa.svg') }}" x="{{ $pumpLineX }}"
                                            y="{{ $pumpLineTopY - 0.3 }}" width="{{ $pumpLineW }}"
                                            height="{{ $pumpLineH + 0.5 }}" preserveAspectRatio="none" opacity="{{ $sOpac }}" />
                                        <image href="{{ asset('sumur/kepala_pompa.svg') }}" x="{{ $pumpHeadX }}"
                                            y="{{ $pumpHeadY }}" width="{{ $pumpHeadW }}" height="{{ $pumpHeadH }}"
                                ...
                                @if ($hasPompa)
                                    ...
                                    <svg style="position:absolute; top:0; left:0; z-index:95;" width="137"
                                        height="{{ $totalH }}" viewBox="0 0 137 {{ $totalH }}" xmlns="http://www.w3.org/2000/svg">
                                        <defs>
                                            <linearGradient id="capGrad-{{ $uid }}" x1="0%" y1="0%" x2="100%" y2="0%">
                                                <stop offset="0%"   stop-color="white" />
                                                <stop offset="50%"  stop-color="#C0C4CA" />
                                                <stop offset="100%" stop-color="#B3B8BF" />
                                            </linearGradient>
                                        </defs>
                                        <rect x="{{ $capX }}" y="{{ $capHeadY }}" width="{{ $capW }}" height="{{ $capHeadH }}"
                                            rx="1.5" ry="1.5" fill="url(#capGrad-{{ $uid }})" stroke="#959CA3"
                                            stroke-width="0.5" opacity="{{ $pOpac }}" />
                                    </svg>
                                @endif
                            </div>
                        </div>
                        <div class="mt-1 text-center text-[10px] font-semibold text-slate-500">
                            Kedalaman Air Tanah<br>
                            <span class="font-bold text-slate-700">{{ $kdlSumur }}</span> m
                        </div>
                    </div>
                    ...
                    <div style="position:absolute; left:0; top:{{ $datAirY }}px; width:{{ $cW }}px; z-index:60;"
                        ...
                        <div class="text-[9px] font-bold uppercase tracking-wide {{ $isOnline ? 'text-stone-700' : 'text-slate-500' }}">DATA AIR TANAH</div>
                        <div class="flex items-baseline gap-1">
                            <span class="text-lg font-extrabold {{ $isOnline ? 'text-slate-900' : 'text-slate-500' }}">{{ $dataAir ?? '-' }}</span>
                    ...
                    <div style="position:absolute; left:0; top:{{ $elevSensY }}px; width:{{ $cW }}px; z-index:60;"
                        ...
                        <div class="text-[9px] font-bold uppercase tracking-wide {{ $isOnline ? 'text-rose-700' : 'text-slate-500' }}">ELEVASI SENSOR</div>
                        <div class="flex items-baseline gap-1">
                            <span class="text-lg font-extrabold {{ $isOnline ? 'text-slate-900' : 'text-slate-500' }}">{{ $lg?->jiat?->kedalaman_sensor ?? '-' }}</span>
                            <span class="text-xs font-semibold text-slate-400">m</span>
                        </div>
                    </div>

                    {{-- RIGHT: MUKA AIR TANAH --}}
                    <div style="position:absolute; right:0; top:{{ $mukaAirY }}px; width:{{ $cW }}px; z-index:60;"
                        class="rounded-lg border-2 px-2 py-1.5 {{ $isOnline ? 'border-sky-300 bg-sky-50' : 'border-slate-300 bg-slate-100 grayscale' }}">
                        <div class="text-[9px] font-bold uppercase tracking-wide {{ $isOnline ? 'text-sky-700' : 'text-slate-500' }}">MUKA AIR TANAH</div>
                        <div class="flex items-baseline gap-1">
                            <span class="text-lg font-extrabold {{ $isOnline ? 'text-slate-900' : 'text-slate-500' }}">{{ $mukaAir ?? '-' }}</span>
                            <span class="text-xs font-semibold text-slate-400">m</span>
                        </div>
                    </div>
                    ...
                    <div style="position:absolute; right:0; top:{{ $elevPompaY }}px; width:{{ $cW }}px; z-index:60;"
                        ...
                        <div class="text-[9px] font-bold uppercase tracking-wide {{ $isOnline ? 'text-amber-700' : 'text-slate-500' }}">ELEVASI POMPA</div>
                        <div class="flex items-baseline gap-1">
                            <span class="text-lg font-extrabold {{ $isOnline ? 'text-slate-900' : 'text-slate-500' }}">{{ $lg?->jiat?->kedalaman_pompa ?? '-' }}</span>
                            <span class="text-xs font-semibold text-slate-400">m</span>
                        </div>
                    </div>
