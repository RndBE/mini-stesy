<div class="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
    ...
    @php
        // Ambil nilai sensor AWGR dari kolom di $latest
        $sensorMap = [
            ...
            'conductivity' => ['label' => 'CONDUCTIVITY', 'satuan' => 'μS/cm', 'icon' => 'icons/awgr/conductivity.svg'],
        ...
        // Cari nilai pH dan suhu air untuk tampilan pH meter
    ...
    <div class="flex flex-col lg:flex-row gap-4 p-5">
        ...
        <div
            ...
            {{-- <div class="relative w-28 select-none">

                <div class="relative rounded-2xl overflow-hidden shadow-md"
                    style="background: linear-gradient(160deg, #e8ec8a 0%, #c9cc5a 40%, #b5b84d 100%); width: 112px; height: 200px;">


                    <div class="absolute"
                        style="top:20px; left:18px; width:76px; height:80px; background:#c9dcb0; border-radius:6px; border:2px solid #8aa068; box-shadow:inset 0 2px 4px rgba(0,0,0,0.15);">
                        <div class="flex flex-col items-center justify-center h-full">
                            <div class="text-[9px] font-bold text-slate-600 tracking-wide">pH</div>
                            <div
                                class="text-2xl font-extrabold text-slate-800 leading-tight {{ $muted ? 'opacity-50' : '' }}">
                                {{ $phDisplay }}
                            </div>
                            <div class="text-[10px] font-semibold text-slate-600 {{ $muted ? 'opacity-50' : '' }}">
                                {{ $suhuDisplay }}<span class="text-[8px]">°C</span>
                            </div>
                        </div>
                    </div>


                    <div class="absolute flex gap-3 justify-center" style="bottom: 28px; left: 0; right: 0;">
                        <div class="rounded-full"
                            style="width:18px; height:18px; background:#d97706; border:2px solid #92400e;"></div>
                        <div class="rounded-full"
                            style="width:18px; height:18px; background:#d97706; border:2px solid #92400e;"></div>
                    </div>


                    <div class="absolute" style="bottom:-28px; left:50%; transform:translateX(-50%);">
                        <div
                            style="width:10px; height:20px; background:#c9cc5a; border-radius:0 0 5px 5px; margin: 0 auto;">
                        </div>
                        <div style="width:4px; height:12px; background:#aaa; border-radius:2px; margin: 0 auto;"></div>
                    </div>
                </div>
            </div> --}}
            {{-- pH Meter assembled from real SVG assets --}}
            <div class="flex flex-col items-center select-none" style="width:73px">

                {{-- Body utama (atas) sebagai base, bagian-tengah di-overlay di depannya --}}
                <div class="relative w-full">
                    {{-- Layer belakang: body kuning --}}
                    <img src="{{ asset('alat-ukur-air/atas-alat.svg') }}" class="w-full block" alt="">

                    {{-- Layer depan: bagian tengah (layar hitam) overlay di atas body --}}
                    <div class="absolute" style="top:10%;left:50%;transform:translateX(-50%);width:60px">
                        <div class="relative">
                            ...
                            {{-- Layar LCD: overlay nilai pH & suhu --}}
