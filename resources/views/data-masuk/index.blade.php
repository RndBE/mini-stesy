...
@section('content')
    <div x-data="dataMasukManager()" class="space-y-6">
        ...
        <div class="overflow-visible rounded-2xl bg-white shadow-sm ring-1 ring-slate-200 p-8">
            <div class="mb-6">
                <h2 class="text-lg font-bold text-slate-900">Pencarian Data</h2>
                <p class="text-sm text-slate-500 mt-1">Pilih logger dan tanggal untuk melihat data sensor yang tersimpan</p>
            </div>
        ...
        <div x-show="dataLoaded" x-cloak class="grid grid-cols-1 md:grid-cols-4 gap-4 animate-in fade-in">
            <div class="overflow-hidden rounded-2xl bg-gradient-to-br from-blue-50 to-blue-100 shadow-sm ring-1 ring-blue-200 p-6">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-xs font-semibold text-blue-600 uppercase tracking-wide">ID Logger</p>
                        <p class="text-3xl font-bold text-blue-900 mt-2" x-text="filters.logger_id"></p>
                    </div>
            ...
            <div class="overflow-hidden rounded-2xl bg-gradient-to-br from-purple-50 to-purple-100 shadow-sm ring-1 ring-purple-200 p-6">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-xs font-semibold text-purple-600 uppercase tracking-wide">Tanggal</p>
                        <p class="text-3xl font-bold text-purple-900 mt-2" x-text="filters.tanggal"></p>
                    </div>
            ...
            <div class="overflow-hidden rounded-2xl bg-gradient-to-br from-emerald-50 to-emerald-100 shadow-sm ring-1 ring-emerald-200 p-6">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-xs font-semibold text-emerald-600 uppercase tracking-wide">Total Data</p>
                        <p class="text-3xl font-bold text-emerald-900 mt-2" x-text="tableData.length"></p>
                    </div>
            ...
            <div class="overflow-hidden rounded-2xl bg-gradient-to-br from-orange-50 to-orange-100 shadow-sm ring-1 ring-orange-200 p-6">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-xs font-semibold text-orange-600 uppercase tracking-wide">Kelengkapan</p>
                        <p class="text-3xl font-bold text-orange-900 mt-2" x-text="dataCompleteness + '%'"></p>
                    </div>
        ...
        <!-- Data Table -->
        <div x-show="dataLoaded" x-cloak class="overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-slate-200 animate-in fade-in">
            <div class="px-6 py-5 bg-gradient-to-r from-slate-50 to-slate-100 border-b border-slate-200 flex justify-between items-center">
                <div>
                    <h3 class="font-bold text-slate-900 text-lg">Data Detail Sensor</h3>
                    <p class="text-xs text-slate-500 mt-1">Tampilan lengkap data sensor untuk periode yang dipilih</p>
                </div>
                <button @click="exportToExcel()"
                    class="inline-flex items-center gap-2 rounded-lg bg-gradient-to-r from-green-500 to-green-600 px-4 py-2.5 text-sm font-semibold text-white hover:shadow-lg hover:shadow-green-500/20 transition-all duration-200 active:scale-95">
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left text-sm text-slate-600 whitespace-nowrap">
                    <thead class="bg-slate-50 text-xs font-semibold uppercase text-slate-700 sticky top-0 border-b border-slate-200">
                        <tr>
                        ...
                        <template x-for="(row, idx) in tableData" :key="idx">
                            <tr class="hover:bg-slate-50 transition-colors">
                                <td class="px-6 py-4 font-medium text-slate-900">
                                    <span class="inline-flex h-8 w-8 items-center justify-center rounded-full bg-slate-100 text-xs font-semibold text-slate-700" x-text="idx + 1"></span>
                                </td>
                                ...
                                <template x-for="(col, colIdx) in columns" :key="colIdx">
                                    <td class="px-6 py-4 text-center text-sm"
                                        :class="row[getColumnKey(col)] === 0 ? 'bg-red-50 text-red-700 font-semibold' : 'text-slate-600'">
                                        <span
                        ...
                        <template x-if="tableData.length === 0 && dataLoaded">
                            <tr>
                                <td :colspan="columns.length + 2" class="px-6 py-12 text-center">
                                    ...
                                    <p class="text-sm text-slate-500">Tidak ada data tersedia untuk tanggal yang dipilih</p>
                                </td>
                            </tr>
                        </template>
                    </tbody>
        ...
        <div x-show="!dataLoaded && !loading" x-cloak class="text-center py-16 px-6">
            <div class="flex justify-center mb-6">
                <div class="relative h-24 w-24 rounded-full bg-gradient-to-br from-indigo-100 to-indigo-50 ring-4 ring-indigo-200 flex items-center justify-center">
            ...
            <p class="text-slate-500 mb-8 max-w-md mx-auto">Pilih logger dan tanggal, kemudian klik tombol "Cari" untuk menampilkan data sensor yang tersimpan.</p>
            <div class="inline-flex gap-3 bg-blue-50 px-6 py-4 rounded-xl ring-1 ring-blue-200">
                ...
                <div class="text-left">
                    ...
                    <p class="text-xs text-blue-700 mt-1">Pilih komponent logger dan tanggal untuk melihat data sensor real-time</p>
                </div>
            </div>
        </div>
    </div>
